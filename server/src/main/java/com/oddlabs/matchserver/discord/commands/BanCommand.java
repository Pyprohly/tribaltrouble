...
public class BanCommand extends DiscordCommand {
    ...
    public ApplicationCommandRequest getCommand() {
        ApplicationCommandOptionData nick_option = ApplicationCommandOptionData.builder().name(
                command_option_nick).description("The in-game profile nick whose account to ban").type(
        ...
        return ApplicationCommandRequest.builder().name(command_name).description(
                command_description).defaultMemberPermissions(BAN_MEMBERS_PERMISSION).addOption(nick_option).addOption(
                        reason_option).build();
    }
}
